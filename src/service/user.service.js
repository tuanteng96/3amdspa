...
class UserService {
    login(username, password) {
        return http.get(`/app/index.aspx?USN=${username}&PWD=${password}&cmd=authen2`);
    }
    QRCodeLogin(qrcode) {
    ...
    getInfo() {
        return http.get(`/app/index.aspx?cmd=authen2&token=${getToken()}`)
    }
    getSubscribe(usn, isUser, token) {
        return http.get(`/app/index.aspx?cmd=authen2&USN=${usn}&token=${token}&IsUser=${isUser}`);
    }
    getListTagService(username, password, memberid) {
        return http.get(`/services/preview.aspx?a=1&token=${getToken()}&cmd=loadOrderService23&MemberID=${memberid}&IsMember=1&fromOrderAdd=0`);
    }
    getBarCode(memberid) {
    ...
    getOrderAll2() {
        return http.get(`/app/index.aspx?cmd=orders23&token=${getToken()}&IsUser=0`)
    }
    getConfig(name) {
